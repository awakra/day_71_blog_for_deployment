...
def register():
    ...
    if form.validate_on_submit():
        ...
        if existing_user:
            # If user exists, flash a message and redirect to login
            flash("You've already signed up with that email. Please log in.")
            return redirect(url_for("login"))
        new_user = User(
            email = form.email.data,
            password = generate_password_hash(form.password.data, method='pbkdf2:sha256', salt_length=8), # Hash the password for security
            name = form.name.data
        )
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and check_password_hash(user.password, form.password.data):
            ...
            # If login fails, flash an error message and redirects to login
